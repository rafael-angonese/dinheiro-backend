...
export class CreateTransactionController {
    async handle(request: Request, response: Response): Promise<Response> {
        ...
        const transaction = await createService.execute({ date, description, amount: Number(amount), type, category_id, account_id, bank_account_id, user_id });
        ...
        if (files) {
            for (const element of files) {
                ...
                await createFileOnTransactionService.execute({ file_id: file.id, transaction_id: transaction.id })
            }
        }
